...
!function( $ ) {
    ...
    GeoMap.setCenter = function( long, lat, zoom ) {
        var center = new OpenLayers.LonLat(long, lat).transform(new OpenLayers.Projection("EPSG:4326"), this._map.getProjectionObject());
        this._map.setCenter(center, zoom || 6);
    };
    ...
    GeoMap.featureSelect = function(evt) {
        ...
        if (self._popup_callback) {
            setTimeout(function() {
                self._popup_callback.apply(self, ['geo-popup-cluster', cluster]);
            }, 10);
        } else {
            setTimeout(function() {
                var ids = [];
                for (var i=0, len=cluster.length; i<len; i++) {
                    ids.push(cluster[i].data.id);
                }
                z_notify("geomap_cluster", {
                            ids: ids,
