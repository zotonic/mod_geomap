    ...
    $.widget( "ui.geomap", {
        ...
        _init: function() {
            ...
            function boundingGeom(features) {
                var coords = features.map(x => x.getGeometry().getCoordinates());
                var maxX = coords.map(x => x[0]).reduce(function(a, b) {
                    return Math.max(a, b);
                });
                var maxY = coords.map(x => x[1]).reduce(function(a, b) {
                    return Math.max(a, b);
                });
                var minX = coords.map(x => x[0]).reduce(function(a, b) {
                    return Math.min(a, b);
                });
                var minY = coords.map(x => x[1]).reduce(function(a, b) {
                    return Math.min(a, b);
                });
